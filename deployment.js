...
const fs = require('fs')
// config.setCurrentContext('pujitm2009-gmail-com@stackathon')

const file = fs.readFileSync('./crd.yaml', 'utf8')
const crd = YAML.parseAllDocuments(file)
...
async function main() {
    try {
        ...
        //
        try {
            await client.apis['apiextensions.k8s.io'].v1beta1.customresourcedefinitions.post({
                body: crd
            })
        } catch (err) {
            //
